# Istruzioni per l'agente Replit — Stadio 1: sorgente unica di navigazione

Crea il modulo che fa da sorgente unica della navigazione (tipi + builder puro
+ hook). È SOLO un file nuovo: nessun'altra modifica, niente UI, niente DB.
Nessun renderer lo consuma ancora — è la base per gli stadi successivi (sidebar
desktop e bottom-bar mobile), quindi a rischio quasi nullo.

Non committare/pushare: lascia nel working tree.

---

## CREA `artifacts/mister-web/src/lib/nav-model.ts`
...
- Nessun cambiamento visibile nell'app (il modulo non è ancora usato).

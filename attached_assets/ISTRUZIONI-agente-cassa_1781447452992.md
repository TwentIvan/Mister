# Istruzioni per l'agente Replit — aggiunta sottosistema cassa (economy)

Applica ESATTAMENTE le modifiche descritte qui sotto al monorepo. Sono tutte
**additive**: non rimuovere né riscrivere codice esistente, non toccare altri
file. Le modifiche sono già state typecheckate (0 errori) fuori dal repo.

Riepilogo: 1 file nuovo, 1 riga aggiunta a `index.ts`, 4 inserzioni puntuali in
`leagues.ts`. Alla fine un comando per creare la tabella nel DB.

---

## 1) CREA il file nuovo `lib/db/src/schema/economy.ts`
...
## 2) MODIFICA `lib/db/src/schema/index.ts`

Aggiungi in fondo al file questa riga (se non già presente):
...
## 3) MODIFICA `lib/db/src/schema/leagues.ts` — 4 inserzioni additive

### 3a) Inserisci le nuove interfacce
...
e IMMEDIATAMENTE PRIMA di quella riga inserisci questo blocco:
...
### 3b) Aggiungi il campo `economy` all'interfaccia `LeagueConfig`

Nell'interfaccia `LeagueConfig`, dopo la riga:
...
### 3c) Inserisci la costante `DEFAULT_ECONOMY_CONFIG`
...
### 3d) Aggiungi `economy` al valore `DEFAULT_LEAGUE_CONFIG`

Dentro l'oggetto `DEFAULT_LEAGUE_CONFIG`, subito dopo il blocco
`postAcquisitionWindow: { ... },` (la sua parentesi graffa di chiusura `},`),
aggiungi come ultima proprietà:
...
## 4) Crea la tabella nel database

Dopo aver salvato i file, esegui dalla shell:

```bash
pnpm --filter @workspace/db push
```

Drizzle proporrà di creare la tabella `economy_transactions` e i tre enum
`currency`, `economy_txn_type`, `economy_txn_status`. Sono tutti oggetti NUOVI:
conferma. Nessuna tabella o colonna esistente viene modificata.

---

## 5) Verifica finale

- `pnpm --filter @workspace/db` typecheck/build deve passare senza errori.
- Le leghe esistenti restano invariate: il campo `economy` vive nel JSONB
  `config` e ha default `DEFAULT_ECONOMY_CONFIG` (cassa interamente spenta).

NON committare e NON fare push: lascia le modifiche nel working tree, così
l'utente le rivede e committa quando vuole.
